...
class HandlersTests(TestCase):
    fixtures = ['fixtures_for_salon.json']
    def setUp(self):
        # Створюємо тестові дані
        self.master1 = Master.objects.first()
        self.master2 = Master.objects.last()
    ...
    def test_specialists_for_week_handler(self):
        ...
        self.assertEqual(response.status_code, 200)
        expected_specialists = {self.master1, self.master2}
        actual_specialists = set(response.context['specialists'])
        self.assertEqual(expected_specialists, actual_specialists)
